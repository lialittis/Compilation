...
let is_a_ref refs v = List.fold_left (fun b (x, _, _) -> b || (x=v)) false refs

let rec string_of_expr refs e = 
  match e.mexpr_desc with
  | ME_const c -> string_of_const c
  | ME_ident x -> if (is_a_ref refs x) then "!"^x else x
  | ME_mem x -> "mem'."^x
  | ME_unop(op,e1) ->
      sprintf "(%s %s)" (string_of_unop op) (string_of_expr refs e1)
  | ME_binop (op, e1, e2) -> 
      sprintf "(%s %s %s)" 
	(string_of_expr refs e1) (string_of_binop op) (string_of_expr refs e2)
  | ME_if (e1,e2,e3) -> 
      sprintf "@\n    (if %s then@\n       %s@\n     else@\n       %s)"
  (string_of_expr refs e1) (string_of_expr refs e2) (string_of_expr refs e3)
  | ME_ref (cl) -> 
      sprintf "(ref %s)" (string_of_list string_of_const ", ref " cl)
  | ME_when (e1,c,e2) -> assert false (* impossible *)
  | ME_merge (e, c_e_list) ->
      sprintf "@\n    (match %s with\n| %s)"
  (string_of_expr refs e)
  (string_of_list (fun (c, e) -> (string_of_const c)^" -> "^(string_of_expr refs e)) "\n| " c_e_list)
  | ME_tuple el -> 
      sprintf "(%s)" (string_of_list (string_of_expr refs) ", " el)
  | ME_app(f,mem,args) ->
      sprintf "(%s_step mem'.%s (%s))" 
	f mem (string_of_list (string_of_expr refs) ", " args)
  | ME_prim(f,args,_) ->
      sprintf "(%s (%s))" f (string_of_list (string_of_expr refs) ", " args)
  | ME_print(el) -> 
      sprintf "(@\n%s    flush_all())"
	(string_of_list (string_of_print_expr refs) "" el)

and string_of_print_expr refs e =
...
let print_compute refs { meq_patt = p; meq_expr = e } = 
  match e.mexpr_desc with 
    | ME_when(e1, c, e2) -> 
      let string_of_changing_references = 
      begin match e1.mexpr_desc with
        | ME_tuple cexpr -> 
          (string_of_list 
            (fun ((x,_,_), expr) -> x ^ " := " ^ (string_of_expr refs expr) )
             "; " (List.combine p cexpr))
        | _ -> (string_of_list (fun (x,_,_) -> x) ", " p) ^ " := " ^ (string_of_expr refs e1)
      end in

      let mexpr_const = { mexpr_desc = ME_const c;
                          mexpr_type = e2.mexpr_type; } in
      printf "  if (%s) then (%s);@\n" 
      (string_of_expr refs { mexpr_desc = ME_binop(Beq, e2, mexpr_const);
                        mexpr_type = [Tbool]; })
      string_of_changing_references

    | _ -> printf "  let (%s) = %s in@\n" 
          (string_of_list (fun (x,_,_) -> x) ", " p)
          (string_of_expr refs e)
...
let print_step n =
  printf "let %s_step mem' (%s) = @\n" 
    n.mn_name 
    (string_of_list (fun (x,_,_) -> x) ", " n.mn_input_step);
  List.iter (print_compute (snd n.mn_compute)) (fst n.mn_compute);
  List.iter print_update n.mn_update;
  printf "  (%s)@\n" (string_of_list (fun (x,_,_) -> x) ", " n.mn_output_step)
