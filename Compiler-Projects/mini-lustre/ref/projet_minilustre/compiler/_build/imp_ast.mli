...
(** Enregistrement représentant la déclaration de la mémoire interne
    d'un noeud. *)
type mem =
    { fby_mem: typed_clocked_var list;
...
type refs =  typed_clocked_var list
...
and m_expr_desc =
  ...
  | ME_ref of const list
  | ME_when of m_expr * const * m_expr
  | ME_print of m_expr list
  | ME_merge of m_expr * ((const * m_expr) list)

type m_equation =
    { meq_patt: typed_clocked_var list;
      meq_expr: m_expr; }

(** Enregistrement représentant la compilation d'un noeud *)
type m_node =
    { mn_name: string; (** nom du noeud *)
      mn_input_step: typed_clocked_var list; (** liste des entrées *)
      mn_output_step: typed_clocked_var list;  (** liste des sorties *)
      mn_local: typed_clocked_var list;  (** liste des variables locales *)
      (* mn_refs: acc_ref;  *)(** accumulations des références *)
      mn_mem: mem; (** déclaration de la mémoire du noeud *)
      mn_init: init; (** initialisation de la mémoire du noued *)
      mn_compute: (m_equation list) * refs; (** calcul de la valeur des flots *)
