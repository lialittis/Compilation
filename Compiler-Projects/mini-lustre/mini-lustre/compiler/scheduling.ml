...
let rec add_vars_of_exp s {texpr_desc=e} =
  ...
  | TE_fby (e1, e2) -> let add_vars_of_exp s e1 in add_vars_of_exp s e2 (*if S.mem e1 s then add_vars_of_exp s e1 else add_vars_of_exp s e2 *)(* TODO *)
  | TE_unop (_,e') -> add_vars_of_exp s e'
  | TE_binop (_,e1,e2) ->
      let s = add_vars_of_exp s e1 in
      add_vars_of_exp s e2
  | TE_if(e1,e2,e3) -> if e1 then add_vars_of_exp s e3 else add_vars_of_exp s e3 (* DONE *)
